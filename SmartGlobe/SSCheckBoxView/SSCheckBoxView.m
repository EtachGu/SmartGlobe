             ...
             checked:(BOOL)aChecked
{
    ...
    l.textColor = RgbHex2UIColor(0x00, 0x00, 0x00);
...
-(void) setIndex:(NSInteger)index
{
    ...
    {
        NSLog(@"set Layer index error : the value should not be negative !");
    }
                            ...
                            checked:(BOOL)isChecked
{
    ...
    switch (s) {
        ...
        case kSSCheckBoxViewStyleMono:
            imageName = @"cb_mono_";
            break;
        case kSSCheckBoxViewStyleMonoBlue:
            imageName = @"cb_mono_blue_";
            break;
        case kSSCheckBoxViewStylesLabel:
            imageName =@"label_small.png";
        default:
            return nil;
    }
    if (s!=kSSCheckBoxViewStylesLabel)
    {
...
-(void)TapOnChechBox: (CGPoint) point
{
    CGRect validTouchArea = CGRectMake((self.frame.origin.x - 5),
                                       (self.frame.origin.y - 10),
                                       (self.frame.size.width + 5),
    ...
    {
        checked = !checked;
        if (style != kSSCheckBoxViewStyleBox)
        {
             [self updateCheckBoxImage];
        }
        if (delegate && stateChangedSelector) {
